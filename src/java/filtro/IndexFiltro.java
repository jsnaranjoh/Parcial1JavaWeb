...
public class IndexFiltro implements Filter{
    ...
    public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain) throws IOException, ServletException {
        ...
        if(tipo!=null)
        {
            ...
            {
               ((HttpServletResponse)response).sendRedirect("/Parcial1JavaWeb/faces/planta/gestionProyectos.xhtml");
            }else{
                if(tipo.equals("contratista")){  
                    ((HttpServletResponse)response).sendRedirect("/Parcial1JavaWeb/faces/contratista/gestionDependientes.xhtml");
                }else{
